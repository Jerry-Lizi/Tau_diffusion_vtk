# -*- coding: utf-8 -*-
"""
Created on Tue Dec 26 20:23:46 2023
@author: Jerry Li
Description: 为2.2_new_simple_multi.py的更新。
ChangedParts: 
    将标量值映射 绘制部分进行更新。提前获取相关的obj模型的各个三角形的信息。
    加快映射、绘制图像的速度。
...
# 需要用到的obj文件夹路径：
ObjFiles_FolderName = 'ObjFilesSplit_all/' #ObjFilesSplit14/'  #'ObjFilesSplit_all/'
# 用于动画制作的关键帧存储路径：
frame_path = "AnimationFrames/"
# tau蛋白扩散的相关系数文件夹路径：
TauCoefficients_FolderName ='Tau_coefficients_all/' # 'Tau_coefficients14/' #Tau_coefficients_all
# 欧几里得项系数改变：
...
df = pd.read_csv('objFiles_sorted_name.csv') #'objFiles_sorted_name.csv'
...
def scalar_map_draw(obj_contents, renderer, renderWindow):
    ...
    for obj_name, scalar_values in obj_contents.items():
        ...
        # 避免重复读取文件
        if obj_name not in polydata_dict:  
            reader = vtk.vtkOBJReader()
            reader.SetFileName(obj_path)
            reader.Update()
            # 获取几何数据
            #polydata = reader.GetOutput()
            polydata_dict[obj_name] = reader.GetOutput()
