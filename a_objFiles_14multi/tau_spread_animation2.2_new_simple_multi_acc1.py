# -*- coding: utf-8 -*-
"""
Created on Fri Dec 29 14:05:57 2023
@author: Jerry Li
Description: 
    1.对代码进行修改，以便进行并行化运行 和 GPU加速
    2.先使用14个obj模型进行测验。之后再用完整的160个obj来做。
ChangedParts:
...
'''
# 进行CPU并行处理1：
def compute_region(i, obj_content):
    file_name_i = file_names[i]
    region_content_i = obj_content[file_name_i]
    ...
    return {
        'region_internal_concentrations': region_internal_concentrations,
        'external_euclidean_concentrations': external_euclidean_concentrations,
        'external_retrograde_concentrations': external_retrograde_concentrations,
        'external_anterograde_concentrations': external_anterograde_concentrations,
        'region_external_concentrations': region_external_concentrations,
        'region_concentrations': region_concentrations
    }
...
def spread_iteration(obj_content):
    cpu_cores = multiprocessing.cpu_count()  # 获取CPU核心数

    with ProcessPoolExecutor(max_workers=cpu_cores) as executor:
        futures = {executor.submit(compute_region, i, obj_content): i for i in range(len(file_names))}
        for future in futures.as_completed():
            i = futures[future]
            result = future.result()
            file_name_i = file_names[i]
            obj_content[file_name_i].update(result)
