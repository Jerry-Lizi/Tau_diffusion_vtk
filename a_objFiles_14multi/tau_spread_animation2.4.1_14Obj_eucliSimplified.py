# -*- coding: utf-8 -*-
"""
Created on Mon Dec 25 21:31:00 2023
@author: Jerry Li
Description:  节省内存的，考虑区域整体j对区域i的各个三角形之间的距离。对跨区域的欧几里得项的修改，减少内存消耗
ChangedParts: 先读取14个的，更改一下读取文件的模式。
1.calculate_euclidean_sigmoid_coefficient函数中，返回一个值。即在计算顺行、逆行扩散系数时，也考虑距离过远时为0的情况。
2.去掉了计算区域i和区域j之间各个三角形之间距离的函数。
3.保存图片时使用1280x720的分辨率
...
NUM_INJECTED = 1
# 如果要进行注射，每个三角形的初始tau蛋白浓度。
initial_concentration = 1/200
# 需要用到的obj文件夹路径：
ObjFiles_FolderName = 'ObjFiles_allSplit/'  #'ObjFilesSplit/'
# 用于动画制作的关键帧存储路径：
...
minValue = 0
maxValue = 0.0001
color_value = 1000 # 映射的精度
iteration_Number = 25 #迭代次数
# 设定浓度的上限值
#concentractions_threshold = 10e+5
#max_concentration = 200  
# 原始方程中 关于时间t的相关系数
t_t = -0.1269745 #-0.0052
...
df = pd.read_csv('objFiles_sorted_name.csv')
...
#printIterations = [1] + [i * 10 + 4 for i in range(500)]
# 构建tau蛋白扩散的迭代n次的函数
