# -*- coding: utf-8 -*-
"""
Created on Mon Jan  1 09:44:50 2024
@author: Jerry Li
Description: 
ChangedParts:
    1.在read_obj_file函数部分就将标量值映射 绘制部分进行更新。提前获取相关的obj模型的各个三角形的信息。
    2.使用4k分辨率保存图像。
    3.使用三种颜色分别绘制。
    4.在控制最终绘制图片分辨率时，设置全局变量：windowSize = (4096, 2160)
    5.在绘制时修改了透明度：alpha = min(abs(value) / max(abs(minValue), maxValue), max_alpha)。确保在0.15以下。
    6.试了下 去掉线框 的效果。
...
D:\毕设_tau蛋白扩散\BrainModel_Draw\Obj_Draw\Python_draw\OBJ_draw_vtk\a_ObjFiles_split\a_objFiles_14multi\tau_spread_animation2.2_new_simple_multi2.py
"""
import vtk
import numpy as np
import pandas as pd
...
# 设置渲染窗口的尺寸为1080p:1920, 1080-- 2k: 2048, 1080-- 4k: 4096, 2160
windowSize = (4096, 2160)
...
def scalar_map_draw(obj_contents, renderer, renderWindow):
    ...
    for i in range(color_value):
        ...
        # 根据标量值计算颜色的饱和度
        saturation = abs(value) / max(abs(minValue), maxValue)

        # 假设基础颜色为紫色 (65/255, 20/255, 69/255)
        base_r, base_g, base_b = 16/255, 43/255, 106/255
        r = base_r * saturation
        g = base_g * saturation
        b = base_b * saturation
        
        alpha = fixed_alpha  # 使用固定的透明度值
        """
        # 使用绿色的不同阴影表示不同的标量值
        ...
        """
        # 使用黄色的不同阴影表示不同的标量值
        # 由于黄色是红色和绿色的组合，因此调整红色和绿色分量
        factor = abs(value) / max(abs(minValue), maxValue) # 根据标量值计算颜色的深浅
        r = 1.0 * factor # 红色分量
        g = 1.0 * factor # 绿色分量
        b = 0.0         # 黄色中蓝色分量通常为0
        """
        ...
        # 使用红色的不同阴影表示不同的标量值
        factor = abs(value) / max(abs(minValue), maxValue) # 根据标量值计算颜色的深浅
        r = 1.0 * factor # 红色分量最大化
        g = 0.0          # 绿色分量保持较低
        b = 0.0          # 蓝色分量保持较低
        '''
        '''
        # 使用青蓝色的不同阴影表示不同的标量值
        # 青蓝色的基本RGB值为 (16/255, 43/255, 106/255)
        #base_r, base_g, base_b = 16/255, 43/255, 106/255
        # 使用紫色的不同阴影表示不同的标量值
        # 紫色的基本RGB值为 (65/255, 20/255, 69/255)
        base_r, base_g, base_b = 65/255, 20/255, 69/255
        factor = abs(value) / max(abs(minValue), maxValue) # 根据标量值计算颜色的深浅
        r = base_r * factor
        g = base_g * factor
        b = base_b * factor
